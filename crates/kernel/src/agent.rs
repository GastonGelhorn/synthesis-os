...
impl BaseAgent {
    ...
    async fn run(
        ...
        // ── Steps 1+2: Fetch memories AND Tool RAG IN PARALLEL ──
        // Both now use local embeddings (~2ms each), but the syscall round-trips
        // still benefit from concurrent execution.
        ...
        // NOTE: Prefetch system removed. The LLM decides which tools to call.
        // Previously, get_volume/get_brightness were auto-executed when Tool RAG
        // included them, causing irrelevant tool calls (e.g. get_brightness for a kite question).
        // Now the LLM reasons first and calls tools only when explicitly needed.
        ...
        loop {
            ...
            if !tool_json.is_empty() {
                ...
                let _ = self.emit_status("THINKING", "Procesando resultados...");
