...
export function useUserSyncState(options: UseUserSyncStateOptions) {
    ...
    // Any change in the workspace (local or applied from server) updates the timestamp.
    useEffect(() => {
        ...
        apiGetSyncState()
            .then(async (state) => {
                ...
                if (state.workspace && typeof state.workspace === "object") {
                    ...
                    // ── Protection: do not overwrite recent local workspace or in-flight tasks/agents ──
                    const now = Date.now();
                    ...
                    // Even if we don't apply the snapshot, we mark that the server workspace is available.
